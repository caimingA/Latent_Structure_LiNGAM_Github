...
class LSLiNGAM():
    ...
    def fit(self):
        ...
        self.cluster_merge()
        self.clusters_record = copy.deepcopy(self.clusters)
        ...
        if len(self.clusters) == 1:
            ...
            ##############
            for c in range(len(self.clusters_record)):
                if len(self.clusters_record[c]) >= 3:
                    temp_observed_edges_dict = dict()
                    X_cluster = self.X[:, self.clusters_record[c]]
                    lingam_R = ReLVLiNGAM.ReLVLiNGAM(X_cluster, highest_l=1)
                    lingam_R.fit(X_cluster)
                    B_hat = lingam_R.B
                    cluster_observed_number = B_hat.shape[0]
                    all_possible_permutations = lingam_R.get_all_possible_permutations()
                    max_num = cluster_observed_number*cluster_observed_number
                    observed_mixing_matrix = B_hat[:, : cluster_observed_number]
                    for perm in all_possible_permutations:
                        print(f"perm: {perm}")
                        temp_B = B_hat[:, perm][:, : cluster_observed_number]
                        print(f"temp_B: {temp_B}")
                        temp_B_inv = LA.inv(temp_B)
                        print(f"temp_B_inv: {temp_B_inv}")
                        observed_temp = np.eye(N=(cluster_observed_number)) - temp_B_inv
                        observed_temp = np.where(np.abs(observed_temp) < 0.25, 0.0, np.abs(observed_temp))
                        edge_num = np.count_nonzero(observed_temp)
                        if edge_num < max_num:
                            max_num = edge_num
                            observed_mixing_matrix = observed_temp
                        ...
                        for j in range(observed_mixing_matrix.shape[1]):
                            if i != j:
                                if observed_mixing_matrix[i][j] != 0:
                                    if self.clusters_record[c][j] in temp_observed_edges_dict.keys():
                                        temp_observed_edges_dict[self.clusters_record[c][j]].append(self.clusters_record[c][i])
                                    else:
                                        temp_observed_edges_dict[self.clusters_record[c][j]] = [self.clusters_record[c][i]] 

                    for item in self.clusters_record[c]:
                        if item in temp_observed_edges_dict.keys() and item not in self.directed_edge_within_observed.keys():
                            self.directed_edge_within_observed[item] = temp_observed_edges_dict[item]
                        elif item in temp_observed_edges_dict.keys() and item in self.directed_edge_within_observed.keys():
                            self.directed_edge_within_observed[item] = temp_observed_edges_dict[item]
                        elif item not in temp_observed_edges_dict.keys() and item in self.directed_edge_within_observed.keys():
                            del self.directed_edge_within_observed[item]
                        else:
                            pass
        ...
        for c_i in range(len(self.ordered_cluster)):
            if len(self.ordered_cluster[c_i]) == 2:
                not_have_edge = test_one_confounder_sixth_robust(self.X[:, self.ordered_cluster[c_i]], self._one_latent_tol)
                i, j = self.ordered_cluster[c_i][0], self.ordered_cluster[c_i][1]
                if not not_have_edge and j not in self.directed_edge_within_observed and i not in self.directed_edge_within_observed:
                    remaining_nodes = range(self.X[:, self.ordered_cluster[c_i]].shape[1]) 
                    cumulants = self._estimate_cumulants(self.X[:, self.ordered_cluster[c_i]])
    ...
    #     _, p_value = Hsic(compute_kernel="gaussian").test(e_triad[:self.n, None], data_centered[:self.n, [2]])
    ...
    def Triad_ind(self, i, j, k):
        ...
        相对原版的改动:
        1. 分母保护: cov(Xj,Xk)统计上不显著异于零时不再直接做除法;
        2. 检验前对Triad残差做二次标准化, 并对双方做秩变换, 消除lognormal重尾影响;
        3. 核函数由polynomial(非特征核)换为gaussian(特征核, hyppo内部用中位数启发式选带宽);
        4. 去掉热循环内的print, 用np.dot替代np.cov避免计算整个2x2矩阵。
        接口与返回值与原版完全一致。
        """
        data_view = self.X[:, [i, j, k]]
        std = data_view.std(axis=0, keepdims=True)
 
        # 退化保护: 常数列无法标准化, 该三元组无检验意义
        if np.any(std < 1e-12):
            return False
 
        data_centered = (data_view - data_view.mean(axis=0, keepdims=True)) / std
 
        n_full = data_centered.shape[0]
        # 标准化后协方差即相关系数; 零相关的95%置信界约为 ±1.96/sqrt(n)
        r_ik = np.dot(data_centered[:, 0], data_centered[:, 2]) / (n_full - 1)
        r_jk = np.dot(data_centered[:, 1], data_centered[:, 2]) / (n_full - 1)
        corr_eps = 1.96 / np.sqrt(n_full)
 
        if np.abs(r_jk) < corr_eps:
            if np.abs(r_ik) >= corr_eps:
                # Xj与Xk不相关而Xi与Xk相关: 任何有限系数都无法消去Xi对Xk的依赖,
                # Triad条件必不成立, 无需检验直接拒绝
                return False
            # 双方都与Xk不相关: 系数取极限值0, 退化为检验 Xi ⊥ Xk
            e_triad = data_centered[:, 0]
        else:
            e_triad = data_centered[:, 0] - (r_ik / r_jk) * data_centered[:, 1]
 
        # 二次标准化: 消除系数放大导致的尺度波动 (对秩变换非必需, 但保持数值稳健)
        e = e_triad[:self.n]
        e_std = e.std()
        if e_std < 1e-12:
            return False
        ...
        # 秩变换: 独立性在严格单调变换下不变, 但可消除重尾对核带宽的破坏
        e_rank = rank_transform_1d(e[:, None])
        xk_rank = rank_transform_1d(data_centered[:self.n, [2]])
 
        _, p_value = Hsic(compute_kernel="gaussian").test(e_rank[:, None], xk_rank[:, None])
        print("p_value: ", p_value)
    ...
    def cluster_edge(self, X):
        ...
        print("confounders: \n", confounders)
        print("confounders: \n", self.directed_edge_within_observed)
...
def cumulant(data, indices):
    """计算高阶交叉累积量 (中心化+方差缩放+跳过单元素块+补偿求和+块矩缓存)"""
    idx = list(indices)
    cols = sorted(set(idx))
    Z = data[:, cols].astype(np.float64)
    sd = Z.std(axis=0)
    Z = (Z - Z.mean(axis=0)) / sd          # 累积量平移不变: 结果不变, 精度大增
    remap = {c: i for i, c in enumerate(cols)}
    idx_local = [remap[i] for i in idx]

    cache = {}                              # 块矩缓存: 同一矩只算一次
    def block_moment(block):
        key = tuple(sorted(block))
        if key not in cache:
            cache[key] = np.prod(Z[:, list(key)], axis=1).mean()
        return cache[key]

    terms = []
    for partition in set_partitions(idx_local):
        if any(len(b) == 1 for b in partition):   # E[Z]=0, 含单元素块的划分整项为零
            continue
        prod_moment = 1.0
        for block in partition:
            prod_moment *= block_moment(block)
        weight = math.factorial(len(partition) - 1) * ((-1) ** (len(partition) - 1))
        terms.append(weight * prod_moment)

    scale = np.prod([sd[remap[i]] for i in idx])  # k次齐次性还原原始尺度
    return math.fsum(terms) * float(scale)
