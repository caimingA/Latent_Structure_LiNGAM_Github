...
class ReLVLiNGAM_once():
    ...
    # j -> i
    def estimate_latent_cumulants(self, j, i, cumulants):
        l = 1
        # highest_l = 1+l
        ...
        print(mean_roots)
        # bs_unmatched = np.full((highest_l), np.nan)
        # bs_unmatched = np.full((1, l), np.nan)
    ...
    def fit(self, X):
        ...
        print(confounders)
        result_c = self.estimate_latent_cumulants(0, 1, cumulants)
        return result_c
        # return confounders
    ...
    def _estimate_num_confounders(self, potential_source, other_node, all_singular_values):
        ...
        for l in range(highest_l+1):
            ...
            #     return l
            if ((1- sigma[r]/np.sum(sigma)) > threshold):
                return l
