...
class LSLiNGAM():
    def __init__(self, X, highest_l, ind_alpha, one_latent_tol, singular_threshold, merge_threshold_first, merge_threshold_next, scale_partly=True, oracle_clusters=None, oracle_detect_within_edges=True, oracle_observed_edges=None):
        ...
        # Oracle模式: 直接指定真实clusters, 跳过聚类识别, 且后续不再合并cluster
        self._oracle_clusters = oracle_clusters
        self._oracle_mode = oracle_clusters is not None
        self._oracle_detect_within_edges = oracle_detect_within_edges
        self._oracle_observed_edges = oracle_observed_edges

    
    def fit(self):
        if self._oracle_mode:
            self.set_oracle_clusters()
            print("clusters (oracle): ", self.clusters)
        else:
                ...
                if len(self.clusters_record[c]) >= 3:
                    ...
                    for item in self.clusters_record[c]:
                        if item in temp_observed_edges_dict.keys() and item not in self.directed_edge_within_observed.keys():
                            ...
                            pass
            self.directed_edge_within_observed = self._oracle_observed_edges
    ...
    def make_result_graph(self):
        ...
        for cluster in self.ordered_cluster:
            ...
            # oracle: incorrect clusters otherwise remove comments
            # for o in cluster:
            #     o_node = GraphNode(f"X{o + 1}")
    ...
    def set_oracle_clusters(self):
        """Oracle模式: 直接使用外部指定的真实clusters, 跳过cluster_Triad/cluster_edge/cluster_merge。
        可选地仍对cluster内部执行观测边检测与定向 (与原cluster_Triad中的逻辑一致)。
        """
        self.clusters = sorted([sorted(list(c)) for c in self._oracle_clusters], key=lambda x: x[0])

        if not self._oracle_detect_within_edges:
            return

        X = self.X
        # 与cluster_Triad中处理聚类内部边的逻辑保持一致 (仅处理前两个元素)
        for c in self.clusters:
            if len(c) != 1:
                not_have_edge = test_one_confounder_sixth_robust(X[:, c[:2]], self._one_latent_tol)
                print("now cluster: ", c)

                if not_have_edge:
                    i, j = c[0], c[1]
                    confounders = self.find_all_confounders(X[:, c[:2]])
                    print("confounders: ", confounders)
                    ...
                    i, j = c[0], c[1]
                    remaining_nodes = range(X[:, c[:2]].shape[1])
                    cumulants = self._estimate_cumulants(X[:, c[:2]])
                    all_singular_values = self._calculate_all_singular_values(remaining_nodes, cumulants)
                    l = 1
                    r = self.constraints[l]["r"]
                    ...
                    if rank_i_to_j < rank_j_to_i:
                        ...
                        if j in self.directed_edge_within_observed:
                            ...
                            self.directed_edge_within_observed[j] = [i]
        self.directed_edge_within_observed = self._oracle_observed_edges
        print("directed edges (oracle within): ", self.directed_edge_within_observed)
    ...
    def find_current_root(self):
        ...
        if cluster_num == 1:
            ...
            if len(roots_possible) == 1 and cluster_num != 2:
                ...
                # if not self._oracle_mode:  # oracle模式下跳过cluster合并
                if True:  # oracle模式下跳过cluster合并
                    for i in range(len(result_list)):
                        if i != res_index:
                            print(f"sub: {np.abs(result_list[i] - result_list[res_index])}")
