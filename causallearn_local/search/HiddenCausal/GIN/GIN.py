...
def GIN(data, indep_test_method='kci', alpha=0.05):
    ...
    while cluster_size < len(var_set):
        ...
        for cluster in combinations(var_set, cluster_size):
            ...
            for z in range(len(remain_var_set)):
                pvals.append(indep_test(data[:, [remain_var_set[z]]], e[:, None], method=indep_test_method))
    ...
    print("cluster list: ", clusters_list)
    ...
    for i in causal_order:
        clusters_set.update(i)
    one_element_set = list(var_set - clusters_set)
    
    # print(one_element_set)
    # if len(one_element_set) >= 2:
    #     random.shuffle(one_element_set)

    # # print(one_element_set)
    one_element_set_order = [[e] for e in one_element_set]
    # causal_order_temp = one_element_set_order + causal_order
    causal_order = one_element_set_order + causal_order
    ...
    latent_adjmatrix = np.zeros((len(causal_order), len(causal_order)))
    ...
    for cluster in causal_order:
        ...
        if len(cluster) == 1:
            ...
            for i in range(one_count, latent_id - 1):
                latent_adjmatrix[latent_id - 1][i] = 1.0
        # id_count += 1
    ...
    return G, causal_order, latent_adjmatrix
...
def GIN_MI(data):
    ...
    min_dep_score_matrix = np.zeros((len(v_set), len(v_set)))
    for (x1, x2) in combinations(v_set, 2):
        x_set = {x1, x2}
        z_set = v_set - x_set
        dep_statistic = cal_dep_for_gin(data, cov, list(x_set), list(z_set))
        min_dep_score_matrix[x1][x2] = dep_statistic
        print(x_set)
        for i in x_set:
            print(dep_statistic)
            print(min_dep_score[i])
    ...
    for (x1, x2) in combinations(v_set, 2):
        x_set = {x1, x2}
        dep_statistic = min_dep_score_matrix[x1][x2]
        for i in x_set:
            if min_dep_score[i] >= dep_statistic:
                min_cluster[i] = x_set
